...
function App() {
        ...
        if (p.first) {
            ...
            if (queryString.id != undefined) {
                ...
                p.likeRef.on("value", function (greetingRef) {
                    ...
                    // debugger
                    if(p.nowDataObjFromFB.nowID == p.noPlayer)
                    {
                        p.checkIfCanActive();
                    }else if(p.nowDataObjFromFB.nowID != p.myId)
                    {
                        p.checkIfCanActive();
                    }else
                    {
                ...
                p.playerEventStart = playerObj.on("value", function (greetingRef) {
                    ...
                    if (nowKeyObj.likeObj) {
                        ...
                        p.nowDataObjFromFB = nowKeyObj.likeObj.nowData;
                    }
    ...
    App.prototype.checkIfCanActive = function (fromClick) {
        ...
        p.snap.ref('/.info/serverTimeOffset')
            ...
            .then(function stv(data) {
                ...
                var offset = data.val();
                var estimatedServerTimeMs = new Date().getTime() + offset;
                p.lastLockTime = estimatedServerTimeMs;
                /*zog(estimatedServerTimeMs)
                zog(finalLocalTime)
                zog(offset)*/
                p.isOpen = true;

                p.openTime = 0;
                if (p.nowDataObjFromFB) {
                    if (p.nowDataObjFromFB.nowID != undefined && p.nowDataObjFromFB.lastLockTime != undefined) {
                        if (p.nowDataObjFromFB.nowID != p.myId && p.nowDataObjFromFB.nowID != p.noPlayer) {
                            p.isOpen = false;
                        }
                        p.openTime = p.lastLockTime - p.nowDataObjFromFB.lastLockTime;
                    ...
                    //p.isOpen = true;
                    var nowTime = (p.waitTimeMiliSecound - p.openTime) / 1000;
                    /*if(nowTime<=0)
                    {
                        p.isOpen = false;
                    }*/
                    p.timerCon.visible = true;
                    p.lockScreenCon.visible = !p.isOpen;

                    p.lockScreenCon.top();
                    p.timerCon.top();

                    p.timerCon.label.text = Math.max(Math.floor(nowTime), 0);

                    stage.update();

                    zogr(p.isOpen, fromClick)
                    ...
                    if (p.isOpen) {
                        if (p.playerObj != undefined) {
                            if (fromClick) {
                                ...
                                var theTimeWhenWeNeedLockAgain = p.nowDataObjFromFB.lastLockTime + p.waitTimeMiliSecound;

                                p.showTimer(p.waitTimeMiliSecound / 1000, true);
                                if (p.nowDataObjFromFB.nowID != p.myId) {

                                }
                                //send to server the time
                                result = p.likeRef.update({ nowData: { nowID: p.myId, lastLockTime: p.lastLockTime } });;
                        ...
                        p.showTimer(Math.floor((p.waitTimeMiliSecound - p.openTime) / 1000));
    ...
    App.prototype.showTimer = function (num, isUp) {
        zog("showTimer", num)
        p.theTimeIsActiveNow = true;
        interval(1, function (obj) {
            ...
            if (num - obj.count == 0) {
                timeout(1, function () {
                    p.theTimeIsActiveNow = false;
                    if (isUp) {
                        result = p.likeRef.update({ nowData: { nowID: p.noPlayer } });;
                        result.then(function () {
                            zog("time data save");
                            ////	console.log("great", key);
                        }).catch(function (error) {
    ...
    App.prototype.getFirstClick = function () {
        stage.firstClick = stage.on("stagemousedown", function () {
            stage.off("stagemousedown", stage.firstClick);
            p.checkIfCanActive(true);
